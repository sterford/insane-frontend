<template>
    <div class="flex flex-col gap-y-3 grow-0 md:px-0 p-4 w-full lg:w-fit">
        <div class="relative border-bottom-custom-adventage flex flex-col md:flex-row items-center bg-custom-black-adventage p-6 gap-y-3 md:gap-x-5 rounded-xl">
            ...
            <img src="@/assets/img/ReviewsAvatars/review1.png" class="w-24 h-24 rounded-full ring-2 ring-[#D4CDFF] ring-offset-4 ring-offset-[#09090D]">
...
const download = () => {
    ...
    if (user.value?.role && allowedRoles.includes(user.value.role)) {
        window.location.href = 'https://Insanedlc.xyz/api/api/v1/loader/files/download/Insaneloader.zip';
    } else {
        notify({
            group: "nahui",
            type: "error",
